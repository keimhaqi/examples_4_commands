...
def get_tagname(pool, tag_id):
    query = "SELECT `tag_name` FROM `tag` WHERE `tag_id`={}".format(tag_id)
    cnx = pool.connection()
    cursor = cnx.cursor()
...
def get_product_tag_user(pool, pid):
    query = "SELECT `user_id`, `tag_id`, `product_id` FROM `user_tag_product` WHERE `delete`=0 AND `product_id`={}".format(pid)
    cnx = pool.connection()
    cursor = cnx.cursor()
    cursor.execute(query)
    ...
    prop = {}
    if cursor.rowcount != 0:
        for (user_id, tag_id, product_id) in cursor:
            tag_name = get_tagname(pool, tag_id)
            if user_id not in prop.keys():
                prop[user_id] = [tag_name]
            else:
                prop[user_id].append(tag_name)
...
def get_all_data_from_product(productUnit, es, esIndex, docType, attrsType, pool1, pool2):
    ...
    queryProduct = "SELECT `product_id`, `category_type_id`, `product_title`, `product_image_link`, " \
                    ...
                    "FROM `product` WHERE `product_id` IN ({}) and `deleted`=0".format(get_tags_product_id(pool1, pool2))
    cnx = pool1.connection()
    cursor = cnx.cursor()
    ...
    if cursor.rowcount != 0:
        ...
        for (product_id, category_type_id, product_title, product_image_link, product_detail,
            ...
            tags = get_product_tag_user(pool1, product_id)
            props = dict(props.items() + tags.items())
...
def get_tags_product_id(pool1, pool2):
    sql = "SELECT `product_id` FROM `user_tag_product`"
    cnx = pool1.connection()
    cursor = cnx.cursor()
    cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='import_product2_pro_with_tag.log', level=logging.DEBUG,
    ...
    count_for_sanity_check = sanity_check_with_error_handler(popular_brands, new_es_name, args.docType, make_query_body_for_brand, count_for_sanity_check)
    count_for_sanity_check = sanity_check_with_error_handler(popular_categories, new_es_name, args.docType, make_query_body_for_categories, count_for_sanity_check)
    count_for_sanity_check = sanity_check_with_error_handler(popular_vendors, new_es_name, args.docType, make_query_body_for_vendors, count_for_sanity_check)
