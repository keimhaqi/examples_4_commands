...
class Parallel(threading.Thread):
    def __init__(self, pool1, esclient, es_index, es_doc_type, product_id):
    ...
    def run(self):
        self.get_top_product_promo_info_vendor_history(self.pool1, self.esclient, self.es_index, self.es_doc_type, self.product_id)

    def make_action(self, id):
        action = {"update": {"_id": str(id)}}
        return json.dumps(action)

    def get_vendor(self, pool, vendor_id):
    ...
    def get_vendor_logo_url(self, pool, vendor_id):
        tmp = {}
        if vendor_id is not None:
            query_vendor = "SELECT `vendor_id`, `vendor_name_en`, `vendor_logo_width`, `vendor_logo_height`, `vendor_logo_url` FROM `vendor` WHERE `vendor_id`={}".format(
                vendor_id)
            cnx = pool.connection()
            cursor = cnx.cursor()
            cursor.execute(query_vendor)
            for vendor_id, vendor_name_en, vendor_logo_width, vendor_logo_height, vendor_logo_url in cursor:
                tmp["vendor_id"] = vendor_id
                tmp["vendor_logo_url"] = vendor_logo_url
                tmp["vendor_name_en"] = vendor_name_en
    ...
    def price_handler(self, product_price):
        if product_price is not None and product_price != "" and product_price != 'N/A':
            if '-' in product_price:
                product_price = product_price.replace(' ', '')
                pc = product_price.split('-')
                min = float(self.get_only_numbers(pc[0].strip()[0:]))
                max = float(self.get_only_numbers(pc[1].strip()[0:]))
                ...
                min = float(self.get_only_numbers(product_price[0:]))
                max = float(self.get_only_numbers(product_price[0:]))
                return {"min": min, "max": max}

    def get_only_numbers(self, nums):
        ret = ''
        for it in nums:
    ...
    def get_top_product_promo_info_vendor_history(self, pool1, es, esIndex, docType, product_id):
        sql = "SELECT `id`, `product_id`, `vendor_id`, `task_id`, `title_en`, `title_ch`, " \
            ...
            " FROM `top_product_promo_info_vendor_history` WHERE `product_id`={} ".format(product_id)
        print(sql)
        ...
        if cursor.rowcount != 0:
            ...
            for (id, product_id, vendor_id, task_id, title_en, title_ch,
                ...
                logging.info("id = {}, product_id = {}, vendor_id = {}, task_id = {}, title_en = {}, title_ch = {}, desc_en = {}, desc_ch = {}, share_title_en = {}, share_title_ch = {}, share_desc_en = {}, share_desc_ch = {}"
                    .format(id, product_id, vendor_id, task_id, title_en, title_ch, desc_en, desc_ch, share_title_en, share_title_ch, share_desc_en, share_desc_ch) + 
                    ...
                    .format(img_url, card_width, card_height, price, start_date, end_date, author_id, keywords, missed_time, created_time, confirmed_time, confirmer_id)
                    )
                ...
                if vendor_id is not None:
                    ...
                    vendor_logo_url = self.get_vendor_logo_url(pool1, vendor_id)
                    if vendor_logo_url is not None and len(vendor_logo_url) > 0 and isinstance(vendor_logo_url, dict):
                        ...
                        if "vendor_logo_height" in keys and vendor_logo_url["vendor_logo_height"] is not None:
                            top_product_promo_info_vendor_history["vendorLogoHeight"] = vendor_logo_url["vendor_logo_height"]
                        if "vendor_name_en" in keys and vendor_logo_url["vendor_name_en"] is not None:
                            top_product_promo_info_vendor_history["vendorNameEn"] = vendor_logo_url["vendor_name_en"]
                            top_product_promo_info_vendor_history["vendors"] = vendor_logo_url["vendor_name_en"]
                        top_product_promo_info_vendor_history["vendorId"] = vendor_id
                ...
                if price is not None:
                    if '-' in price:
                        ...
                        min = None
                        try:
                            min = float(self.get_only_numbers(pc[0].strip()[1:]))
                        except ValueError, e:
                        ...
                        max = None
                        try:
                            max = float(self.get_only_numbers(pc[1].strip()[1:]))
                        except ValueError, e:
                            logging.error('ProductId=' + str(product_id))
                            logging.error(e)
                            continue
                        ...
                        min = None
                        try:
                            min = float(self.get_only_numbers(price[1:]))
                        except ValueError, e:
                            logging.error('ProductId=' + str(product_id))
                            logging.error(e)
                            continue
                        max = None
                        try:
                            max = float(self.get_only_numbers(price[1:]))
                        except ValueError, e:
                            logging.error('ProductId=' + str(product_id))
                            logging.error(e)
                            continue
                ...
                if start_date is not None:
                    try:
                        top_product_promo_info_vendor_history["startDate"] = int(time.mktime(time.strptime(str(start_date), '%Y-%m-%d %H:%M:%S')) * 1000)
                    except ValueError, e:
                        logging.error("start_date = " + str(start_date) + ", " + str(e))
                if end_date is not None:
                    try:
                        top_product_promo_info_vendor_history["endDate"] = int(time.mktime(time.strptime(str(end_date), '%Y-%m-%d %H:%M:%S')) * 1000)
                    except ValueError, e:
                ...
                if missed_time is not None:
                    try:
                        top_product_promo_info_vendor_history["missedTime"] = int(time.mktime(time.strptime(str(missed_time), '%Y-%m-%d %H:%M:%S')) * 1000)
                    except ValueError, e:
                        logging.error("missed_time = " + str(missed_time) + ", " + str(e))
                if created_time is not None:
                    try:
                        top_product_promo_info_vendor_history["createdTime"] = int(time.mktime(time.strptime(str(created_time), '%Y-%m-%d %H:%M:%S')) * 1000)
                    except ValueError, e:
                        logging.error("created_time = " + str(created_time) + ", " + str(e))
                if confirmed_time is not None:
                    try:
                        top_product_promo_info_vendor_history["confirmedTime"] = int(time.mktime(time.strptime(str(confirmed_time), '%Y-%m-%d %H:%M:%S')) * 1000)
                    except ValueError, e:
            ...
            action = self.make_action(id=product_id)
    ...
    while total < num_of_products:
        ...
        if cursor.rowcount != 0:
            for (product_id) in cursor:
                total = total + 1
                if len(product_id) > 0 and isinstance(product_id, tuple):
                    thd = Parallel(pool1, es, esIndex, docType, product_id[0])
                    thd.start()
                    threads.append(thd)
