...
def get_brand_info_from_brands(pool1, pool2, pool3, brand_id, es, indx, docType, field):
    ...
    :param docType:
    """
    queryBrands = "SELECT `brand_id`, `brand_name` , `alias_c1`, `alias_c2`, `alias_e1`, `alias_e2`, `alias_e3` FROM `brands` WHERE `brand_id`={}".format(
        brand_id)
...
def insert_into_Vocabulary(pool, words, field, language, analyzed, sha1, alias, autonym, es, indx, docType):
    ...
    insertSql = "INSERT INTO `SysVocabulary` (`words`, `field`, `sha1`, `channel`, `language`, `analyzed`, `alias`, `autonym`) " \
                "VALUES ('{}', '{}', '{}', {}, '{}', '{}', {}, {})".format(words, field, sha1, 1, language, analyzed,
                                                                           alias, autonym)
...
def get_vendor_infos(pool1, pool2, pool3, vid, es, indx, docType, field):
    queryVendorInfo = "SELECT `vendor_id`, `vendor_name_en`, `vendor_name_ch` FROM vendor WHERE `vendor_id`={}".format(
        vid)
    cnx = pool2.connection()
    cursor = cnx.cursor()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--esIndex', default='autojinbag')
