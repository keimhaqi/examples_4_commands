...
def get_top_product_promo_info(pool1, pool2, pid):
    sql = "SELECT `product_id`, `title_en`, `title_ch`, " \
          " `desc_en`, `desc_ch`, `share_title_en`, `share_title_ch`, `share_desc_en`, `share_desc_ch`, " \
          "`img_url`, `card_width`, `card_height`, `deleted`, `created_time`" \
          " FROM `top_product_promo_info` WHERE `product_id`={} and `deleted`=0".format(pid)
    cnx = pool1.connection()
    cursor = cnx.cursor()
    cursor.execute(sql)
    ...
    if cursor.rowcount != 0:
        for (product_id, title_en, title_ch, desc_en, desc_ch, share_title_en,
             share_title_ch, share_desc_en, share_desc_ch, img_url, card_width, card_height, deleted,
             created_time
             ) in cursor:
            if product_id is not None:
                prop["id"] = product_id
            ...
            for (product_id, category_type_id, product_title, product_image_link, product_detail,
                ...
                if history_low_price is not None and history_low_price.strip() != '' and product_price != 'N/A':
                    if '-' in history_low_price:
                        ...
                        historyLowPriceMin = min
                        historyLowPriceMax = max
                    ...
                    props["historyLowPrice"] = None
                    props["historyLow"] = None
                if confirmed_msrp is not None:
                    props["confirmedMsrp"] = confirmed_msrp
                else:
                    props["confirmedMsrp"] = None
                ...
                if top_product_promo_info_vendor is not None and len(top_product_promo_info_vendor) != 0:
                    ...
                    props["topProductPromoInfoVendor"] = None
                    props["topProduct"] = None
                    props["promo"] = None
                ...
                if top_product_promo_info is not None and len(top_product_promo_info) != 0:
                    ...
                    props["topProductPromoInfo"] = None
...
if __name__ == '__main__':
    ...
    if len(keys) == 0:
        ...
        if es.indices.exists(new_es_name):
        ...
        if not es.indices.exists(index=new_es_name):
            ...
            es.indices.delete(index=new_es_name)
            es.indices.create(index=new_es_name, body=index_mappings)
    ...
    for brand in popular_brands:
        res = es.search(index=new_es_name, doc_type=args.docType, body=make_query_body_for_brand(brand))
        if res is not None and 'hits' in res.keys():
            count_for_sanity_check = count_for_sanity_check + len(res['hits'])

    for category in popular_categories:
        res = es.search(index=new_es_name, doc_type=args.docType, body=make_query_body_for_categories(category))
        if res is not None and 'hits' in res.keys():
            count_for_sanity_check = count_for_sanity_check + len(res['hits'])

    for vendor in popular_vendors:
        res = es.search(index=new_es_name, doc_type=args.docType, body=make_query_body_for_vendors(vendor))
        if res is not None and 'hits' in res.keys():
            count_for_sanity_check = count_for_sanity_check + len(res['hits'])
    ...
    if count >= 3000000L: #and count_for_sanity_check >= data_check_total:
        ...
        if not flag:
            if old_es_name is not None:
                if es.indices.exists(index=old_es_name):
